...
import { CheckoutView, initCheckoutView } from "./views/checkout.js";
...
import { POSView, initPOS } from './views/pos.js';
...
const routes = {
  ...
  "/pos":           { view: POSView, init: initPOS, protected: true, allowedRoles: ['vendedor', 'admin'] } // ← NUEVA RUTA PROTEGIDA
};
...
export function router() {
  ...
  // ============================================================
  //        PROTECCIÓN DE RUTAS POR ROL (NUEVO)
  // ============================================================
  if (route.protected) {
    ...
    if (!userRole || !route.allowedRoles.includes(userRole)) {
      app.innerHTML = `
        <div class="container" style="text-align:center; padding:4rem 2rem;">
          <h2 style="font-size:2rem; margin-bottom:1rem; color:#ef4444;">🔒 Acceso Denegado</h2>
          <p style="color:var(--text-muted); margin-bottom:2rem;">
